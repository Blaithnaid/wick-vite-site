...
const AdminDash = () => {
	...
	return (
		<>
			<Header />
			<div className="w-full h-full bg-oxford-50">
				<div className="pt-32 bg-oxford-50 my-4 text-black flex flex-col items-center">
						...
						{form === "Tips" && (
							...
							>
								<select
									...
									onChange={(e) =>
										setNewTip({ ...newTip, platform: e.target.value })
									}
									className="bg-slate-300 w-full p-2 rounded"
									required
								>
								...
								<input
									...
									value={newTip.youtubeUrl}
									onChange={(e) =>
										setNewTip({ ...newTip, youtubeUrl: e.target.value })
									}
									className="bg-slate-300 w-full p-2 rounded"
									required
								/>
				...
				<div className="min-h-screen bg-oxford-50 pb-16 px-4 max-w-7xl mx-auto">
					<div className="w-full grid md:grid-cols-2 grid-cols-1 gap-4 items-stretch justify-center">
						<div className="w-full rounded-xl min-h-[640px] h-fit pb-2 bg-slate-400">
							...
							<div className="px-2 space-y-2 pt-2">
								{trends.map((trend) => (
									...
									>
										<div>
											<h3 className="font-bold text-left text-gray-600">
												{trend.title}
											</h3>
						...
						<div className="w-full rounded-xl min-h-[640px] h-fit pb-2 bg-slate-400">
							...
							{Object.entries(
								tips.reduce((acc, tip) => {
									if (!acc[tip.platform]) acc[tip.platform] = [];
									acc[tip.platform].push(tip);
									return acc;
								}, {}),
							).map(([platform, platformTips]) => (
								<div key={platform} className="mb-6">
									<h3 className="font-bold text-lg pt-2 text-gray-700">
										{platform}
									</h3>
									<div className="space-y-2 px-2 pt-2">
										{platformTips.map((tip) => (
											<div
												key={tip.id}
												className="bg-slate-300 p-4 relative rounded shadow flex justify-between items-center"
											>
												<div>
